...
class CartService {
  Future cartProductCount(int userId) async {
    Uri wishUrl = Uri.parse('http://localhost:3005/api/cart/count/$userId');
  ...
  Future addToCart(int productId, int storeId, int userId, int quantity) async {
    Uri _URL = Uri.parse('http://localhost:3005/api/cart/add/');
  ...
  Future deleteFromCart(int cart_Id) async {
    Uri _BaseURL = Uri.parse('http://localhost:3005/api/cart/$cart_Id');
